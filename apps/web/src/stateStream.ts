...
function isState(value: unknown): value is RobotState {
  ...
  return state.version === 1 && state.mode === 'mock' && typeof state.connected === 'boolean'
    && typeof state.status === 'string'
    && ['idle', 'running', 'stopping', 'completed', 'stopped', 'faulted'].includes(state.status)
    && count(state.revision) && count(state.completed_commands) && count(state.total_commands)
    && number(state.x_mm) && number(state.y_mm) && number(state.z_mm)
    && number(state.speed_mm_s) && state.speed_mm_s > 0
    && Array.isArray(state.joints_deg) && state.joints_deg.length === 3 && state.joints_deg.every(number)
    && ['released', 'gripped'].includes(state.gripper as string)
    && ['run_id', 'program_name', 'active_command_id', 'active_command_type', 'error']
      .every(key => text(state[key]))
}

export function connectStateStream({ url, onState, onConnection }: {
  url: string
