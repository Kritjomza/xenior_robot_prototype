  ...
  if (body && typeof body === 'object' && 'detail' in body) {
    ...
    if (Array.isArray(detail)) return detail.map(item => {
      if (item && typeof item === 'object' && 'msg' in item) {
        const location = Array.isArray(item.loc) ? item.loc.join('.') : 'program'
...
export async function post<T>(path: string, body?: unknown): Promise<T> {
  ...
  try {
    ...
    if (controller.signal.aborted) {
      throw new Error('Request timed out. Check live state before retrying a run.')
    }
    throw error
...
export function parseProgram(source: string): unknown {
  try {
    ...
    throw new Error(`Invalid JSON: ${error instanceof Error ? error.message : 'check the syntax'}`)
  }
}
