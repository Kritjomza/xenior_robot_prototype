...
function fakeClient(initialSession: Session | null) {
  ...
  return { client, emit: (event: string, next: Session | null) => listener?.(event, next), unsubscribe, signOut }
}
...
it('fails closed with setup guidance when Supabase browser configuration is missing', () => {
  expect(createBrowserSupabaseClient({
    VITE_SUPABASE_URL: 'https://project.supabase.co',
  })).toBeNull()
  expect(createBrowserSupabaseClient({
    VITE_SUPABASE_PUBLISHABLE_KEY: 'sb_publishable_browser_only',
  })).toBeNull()
  render(<AuthProvider client={null}><Consumer /></AuthProvider>)
  expect(screen.getByRole('heading', { name: 'Supabase setup required' })).toBeInTheDocument()
...
it('loads the persistent session and follows subsequent auth state changes', async () => {
  ...
  const view = render(<AuthProvider client={auth.client}><Consumer /></AuthProvider>)
...
it('logs out through Supabase and clears the local authenticated state', async () => {
  const auth = fakeClient(session)
  render(<AuthProvider client={auth.client}><Consumer /></AuthProvider>)
