...
def create_app() -> FastAPI:
    ...
    async def validate(program: RobotProgramV1) -> dict[str, bool | int]:
        return {"valid": True, "command_count": len(program.commands)}

    @app.post("/api/v1/runs", status_code=202)
    async def start(program: RobotProgramV1, request: Request) -> dict[str, str]:
        runtime: Executor = request.app.state.executor
        return {"run_id": await runtime.start(program)}

    @app.post("/api/v1/runs/{run_id}/stop")
    async def stop(run_id: str, request: Request) -> RobotState:
        runtime: Executor = request.app.state.executor
        return await runtime.stop(run_id)

    @app.post("/api/v1/reset")
    async def reset(request: Request) -> RobotState:
        runtime: Executor = request.app.state.executor
        return await runtime.reset()

    @app.get("/api/v1/state")
    async def state(request: Request) -> RobotState:
        runtime: Executor = request.app.state.executor
