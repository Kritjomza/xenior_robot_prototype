...
@pytest.mark.parametrize("value", ["12", True, None, [], float("nan"),
                                       float("inf"), float("-inf")])
...
def test_reject_invalid_numeric_values(field, value):
    if field == "seconds":
        ...
        command = {"type": "move_xyz", "x_mm": 0, "y_mm": 0,
                   "z_mm": -200, "speed_mm_s": 100, field: value}
    with pytest.raises(ValidationError):
        RobotProgramV1.model_validate(program(command))
...
def test_reject_duplicate_ids_empty_program_and_unknown_fields():
    data = program({"type": "home"})
    for invalid in [dict(data, commands=[]), dict(data, mode="robot"),
...
def test_accept_all_commands_and_integer_coordinates():
    commands = [{"type": "home"}, {"type": "set_speed", "speed_mm_s": 50},
                {"type": "move_xyz", "x_mm": 100, "y_mm": 30, "z_mm": -250},
                {"type": "grip"}, {"type": "release"},
                {"type": "wait", "seconds": 0.01}, {"type": "stop"}]
    data = {"version": 1, "name": "all", "commands": [
        {"id": f"c{i}", **c} for i, c in enumerate(commands)]}
